...
public class Labirinto {
	...
	public static LabirintoBuilder newBuilder(String fileName)
	...
	public static class LabirintoBuilder {
		...
		/**
		 * Crea un nuovo attrezzo con i parametri inseriti e lo aggiunge nell'ultima
		 * stanza aggiunta al labirinto.
		 * 
		...
		/**
		 * Aggiunge un collegamento tra stanze. La prima stanza è collegata alla seconda
		 * nella direzione inserita. Automaticamente, collega le stanze in ambi i versi
		 * (Stanza1 collegata a Stanza2 a nord, mentre Stanza2 ha Stanza1 a sud). Se non
		 * esiste una delle due stanze che voglio mettere vicine, semplicemente mi
		 * restituisce il labirinto così com'è.
		 * 
		 * ESEMPIO: ("Cucina", "Atrio", "Nord") -> Cucina è connessa a NORD verso Atrio;
		 * Atrio è connesso a SUD verso Cucina.
		 * 
		 * @param Nome      della stanza1 (che resta ferma) che vuoi collegare a una
		 *                  stanza adiacente.
		...
		public LabirintoBuilder addAdiacenza(String nomeStanzaCorrente, String nomeStanzaDaAdiacentare,
				Direzione direzioneDiAdiacenza) {
			if (mapNome2Stanza.containsKey(nomeStanzaCorrente) && mapNome2Stanza.containsKey(nomeStanzaDaAdiacentare)) { // se
																															// le
																															// stanze
																															// da
																															// mettere
																															...
																															// nostra
																															// mappa
				mapNome2Stanza.get(nomeStanzaCorrente).impostaStanzaAdiacente(direzioneDiAdiacenza,
						mapNome2Stanza.get(nomeStanzaDaAdiacentare));
				mapNome2Stanza.get(nomeStanzaDaAdiacentare).impostaStanzaAdiacente(
						direzioneDiAdiacenza.getDirezioneOpposta(), mapNome2Stanza.get(nomeStanzaCorrente));
			} else
				throw new IllegalArgumentException("Stanze non trovate! Bisogna prima aggiungerle! metodo addStanza()");
			return this;
		}

		/**
		 * Aggiunge una stanza all'elenco delle stanze esistenti, e restituisce la
		 * stanza in questione, per essere usata come variabile di istanza del
		 * LabirintoBuilder su cui lavorare. USARE SOLO DENTRO LABIRINTOBUILDER, NON
		 * INVOCARE AL DI FUORI (MethodChain interrotta altrimenti).
		 * 
		...
		public LabirintoBuilder addStanzaBloccata(String nomeStanzaAggiunta, Direzione nomeDirezioneBloccata,
				String nomeEnabler) {
